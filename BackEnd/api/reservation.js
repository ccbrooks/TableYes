...
module.exports.submitReservation = async (event, context, callback) => {
    ...
    const reservation = reservationInfo(userID, restaurantID, reserveTime, partySize, sort);
...
const reservationInfo = (userID, restaurantID, reserveTime, sort) => {
...
async function updateSort(reservationID) {
    ...
    for(let i = 0; i < reservations.length; i++){
        if(reservations[i+1].submitreservationAt < reservations[i].submitreservationAt){
            
            //unsure how to execute code here

        }
    }
//https://www.geeksforgeeks.org/insertion-sort/
/*function insertionSort(reservations) 
{
    for (i = 0; i < reservations.length; i++)
    { 
        reservations[i].submittedAt = reservations[i]; 
        j = i - 1; 

        while (j >= 0 && reservations[j] > reservations.submittedAt)
        { 
            reservations[j + 1] = reservations[j]; 
            j = j - 1; 
        } 
        reservations[j + 1] = reservations.submittedAt; 
    } 
} 
    console.log("target: ", target);
